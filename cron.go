...
func (c *Cron) debug(format string, v ...interface{}) {
	audit.Debug(format, v...).From(c.CodeVM()).High().Put()
}
	...
	for {
		...
		for {
			...
			case id := <-c.remove:
				...
				now = c.now()
				c.removeEntry(id)
				c.debug("%s %d task removed ", c.Name(), id)
			}
...
func (c *Cron) Clear() {
	for _, entry := range c.entries {
		c.debug("%s cron entry id:%d exit ...", c.Name(), entry.ID)
		c.removeEntry(entry.ID)
	}
}

func (c *Cron) Close() error {
	c.debug("%s close ...", c.Name())
	c.Status = lua.VTClose
	c.Clear()
	c.Stop().Done()
	return nil
}
